/**
 * Componente Dropdown con soporte para carga asíncrona
...
interface DropdownProps {
  /** Items estáticos (hardcoded) */
  items?: DropdownItem[]
  /** Datos cargados asíncronamente (prioriza sobre items) */
  asyncItems?: DropdownItem[]
...
export function Dropdown({
  ...
  const selectedItem = finalItems.find(item => item.value === value)
        ...
        <div className="p-1">
          {isLoading ? (
            <div className="flex items-center justify-center py-4">
              ...
              <span className="ml-2 text-sm text-gray-500 dark:text-gray-400">{loadingMessage}</span>
            </div>
          ) : !hasItems ? (
